...
function initializeModalForm() {
  ...
  document
    .getElementById("modal-start-date")
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      submitBtn.innerHTML =
        '<span style="display: inline-flex; align-items: center; gap: 0.5rem;"><div class="spinner"></div>Submitting...</span>';
      ...
      if (required.some((f) => !payload[f] || !payload[f].trim()))
...
function bookingFormHandler() {
  ...
  if (phoneInput) {
    phoneInput.addEventListener(
      ...
      async function () {
        await loadPhoneLibrary();
        if (window.intlTelInput && !itiInstance)
          itiInstance = initializePhoneInput(phoneInput);
      },
      { once: true },
    );
  }
